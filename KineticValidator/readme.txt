...
Validations implemented:
	...
	7) Duplicate JSON property names - JSON properties with similar names within one JSON object.
	...
	13) Hard-coded message strings - 
	14) Possible strings - property values similar to any of the defined strings. Can be an option to use "{{strings.*}}" instead of hard-coded value.
	...
	17) Empty string values
	18) Redundant strings
	19) Non existing strings
	20) Overriding strings
	...
	23) Overriding events - events overriding each other inside the project files (copy-paste?) and events overriding imported events if imported ones are not empty (have any "type" action).
	24) Redundant events
	25) Non existing events
	26) Empty dataView names - not sure this can happen but it's just an idea
	27) Redundant dataViews
	28) Non existing dataViews
	29) Overriding dataViews
	30) Non existing dataTables
...
Program settings:
	...
	\program_folder\project_folder\ignore.json
		Project ignore list used to filter validations reports for certain project only. Empty field means "any" value.
	\program_folder\ignore.json
		Global ignore list used to ignore specified errors in every project. Empty field means "any" value.
...
Tech. notes:
	...
	- Double-click/select "LineId" column to see compiled file with "LineId" highlighted (helps in case there is no highlight for original files).
